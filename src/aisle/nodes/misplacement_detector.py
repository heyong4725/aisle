...
differs from the item's is misplaced. Consumes the NON-privileged `poses`
topic (identical layout contract to oracle_state, VAL-6-clean). Same
occupancy geometry as the verifier (cannot drift). Pure core, thin dora
main (CON-12)."""
...
def detect_misplacements(poses: np.ndarray, plano: dict, cfg: RetailCfg) -> dict:
