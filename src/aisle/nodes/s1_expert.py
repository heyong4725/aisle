...
def main() -> None:
    ...
    def send_nav(nav_goal: dict) -> None:
        ...
        # here rather than at each `pending = {...}` site because a retry or
        # repark reuses `{**done, ...}`, which would otherwise carry the
        # PREVIOUS leg's id and reject its own reply.
    ...
    for event in node:
        ...
        if event["id"] == "reset_done":
            ...
            if not nav_result_is_current(pending.get("goal_id"), reply_id):
                # issue #179: a result from a leg we are NOT waiting on —
                # most importantly one carried over from a previous episode
                # — must never complete the live subtask. Loud, because the
                # only other symptom is a subtask that silently completes.
                print(
