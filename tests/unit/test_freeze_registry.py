...
def test_shared_cli_successor_preserves_bnd_protocol_and_review_gates():
    ...
    root = REPO_ROOT / "analysis/freeze"
    previous = json.loads((root / "bnd-task-band-calibration-v11/freeze-manifest.json").read_text())
    current = json.loads((root / "bnd-task-band-calibration-v12/freeze-manifest.json").read_text())
    assert current["seed_commitment"] == previous["seed_commitment"]
    for key in (
        "analysis",
        "budgets",
