...
def test_first_graph_shim_snapshots_on_first_validate(tmp_path, monkeypatch):
    """PR #27 r3/r6: the zero-shot artifact is the graph CONSUMED BY THE
    FIRST validate call — the SOURCE shim (installed on the worktree's
    editable cli.py; entry-point wrappers get regenerated by uv) snapshots
    before delegating, once."""
    import textwrap

    from h1_protocol import SHIM_SNIPPET

    graph = tmp_path / "agent_h1.yaml"
    snap = tmp_path / "first_graph.yaml"
    snippet = SHIM_SNIPPET.format(graph=str(graph), snap=str(snap))
    shim_body = textwrap.dedent(snippet.split("def main() -> int:\n", 1)[1])
...
def test_session_cli_failure_preserves_stderr(tmp_path, monkeypatch):
    ...
    scratch = tmp_path / "scratch"
    out = tmp_path / "runs" / "h1"
    (wt / "src" / "aisle" / "harness").mkdir(parents=True)
    (wt / "src" / "aisle" / "harness" / "cli.py").write_text("def main() -> int:\n    return 0\n")
