...
tens of KB while carrying exactly what the identity stage judges.
CON-8: JSON to stdout, logs to stderr, exit 0 iff ok.
"""
...
def main() -> int:
    ...
    try:
        ...
        for goal, high in zip(goals, ends, strict=True):
            ...
            print(
                f"{target}: present @{best_ns / 1e9:.2f}s score {best_score:.4f}", file=sys.stderr
            )
            low = high
