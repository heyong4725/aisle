...
2. **Pos is a 3D error (RS-4, PR #19 review).** The pos criterion
   measures the full 3D distance from the resting template point
   (template xy, board surface + half height) — a floating item fails
   pos even when centered in xy.
3. **Yaw vs front-face are independent axes (RS-4).** The yaw criterion
...
4. **Alignment line = the slot's template front edge.** The "row
...
5. **Overhang** = the item's world extent along the facing axis past the
   unit's front edge (depth/2), with a small numeric epsilon
   (`overhang_tol_m`) for "zero overhang".
6. **Slot occupancy** = xy within `slot_occupancy_radius_m` of the slot
...
7. **S1 counter semantics (RS-7).** Order lines match on the FULL
   (product, spec, qty) triple: an item satisfies a line only if both
   its product AND its true spec disambiguator match, so a line with an
   invalid spec is unfulfillable (never success). The immediate
   extra_item rule keys on the product NAME. On-counter = xy within the counter
...
10. **Deferred:** the HAR-1 rollout `--tier S1|S2|S3` flag lands with T15
