...
their prefix matches a defined spec prefix (so SHA-256/UTF-8 prose is
ignored); citations or waivers of undefined IDs, duplicate IDs, unparseable
test files, and an empty specs/ are all hard errors, so coverage cannot be
faked by typo and the gate cannot silently go green. Process/conduct rules (CON-10,
