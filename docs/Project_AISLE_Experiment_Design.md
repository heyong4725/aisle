...
***Safe Learning*, defined** (added 2026-08-17, issue #269 — the gloss above explained two of the three words and skipped the least self-evident one). There is **no training code in this project**: no `.backward()`, no optimizer, no `def train(`. Four distinct things wear the word *learning*, and only the last is the conventional one. (1) **Outer-loop search by the coding agent** — the primary sense: compose, validate, roll out, read traces, revise, with the validator's error messages as the learning signal. (2) **Accumulation** — the evalcarded skill library that outlives a session (H3). (3) **The reinforcement-learning sense** — AISLE as the *environment* a learner acts in, with a frozen scorer it cannot edit; the verifier emits a status enum plus a failure class, never a scalar reward, so the closest classical framing is held-out model selection rather than RL. (4) **Learned models** — the VLA/VLM/world-model tier of §7.5, inference-only so far. *Safe* is structural rather than aspirational: the tracked artifacts that judge and constrain learning are frozen, while the validator proves scoped oracle-isolation and declared motion-path gating (§4.2 verifier, §4.3 reset, the oracle-isolation and motion-gating validator rules, and the registry floor that forbids a candidate setting its own passing grade). This is not a process-wide bypass claim. The operating definition: **learning in AISLE is a change to the system that is recorded, attributable, and re-runnable** — an improvement leaving no such artifact is indistinguishable from a lucky seed, which is why learning is externalized into typed-graph diffs, evalcarded skills, and idea-tree entries rather than held in weights or context. Worked treatment: technical report §3.5 and §7.6.
...
- **Structural safety, not behavioral safety.** Their safety is budgets and hoping agents behave; ours is type-system-shaped: motion-class gating, guard interposition, unroutable oracle topics. "Agents freely author motion code; validated declared paths traverse the guard, measured guard interventions alter kinematically illegal proposals, and the verifier counts observed wrong-medicine outcomes" (H5, worded per SFE-14: not a by-construction guarantee) answers the objection that blocks agentic robotics from deployment — and pharmacy framing makes it legible to non-robotics audiences.
