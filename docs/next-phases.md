...
| H6 (design doc §6, Operation) | Recorded bounded feasibility: [three retained development cells passed](../analysis/h6/findings.md) on 2026-08-26, using validated relaunch after the lockstep hot-swap machinery finding. One session per fault tier; no logs-only comparator or independent reproduction ([claim scope](claim-evidence.yaml)). A further campaign needs a distinct question and prerequisites; the existing cells do not establish a typed-evidence advantage or general repair rate. |
