...
- M0-2: Re-running M0-1 with identical seeds reproduces a statistically indistinguishable outcome: a two-sided Fisher exact test on the two runs' success counts MUST NOT reject at p < 0.01, and per-seed status flips are logged in the test output (CON-5 layer (d) as amended by ADR-26; the pre-amendment per-episode-vector equality is not a property the Metal backend can promise, issue #71).
