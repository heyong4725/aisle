...
- M0-2: Re-running M0-1 with identical seeds MUST independently satisfy M0-1's pass1 ≥ 0.95 (bounding the rate difference at ≤ 0.04 by construction — non-rejection by a significance test is NOT equivalence and does not gate), and the replicate evidence — both success counts, the per-seed flip set, and a two-sided Fisher exact p-value as context — MUST be persisted to `runs/<rerun-id>/m0_2_replicate.json`. Episode goals MUST be bit-identical across the pair after stripping the timing-layer `reset_sim_ns` field (CON-5 layers (a)/(d) as amended by ADR-26; per-episode status-vector equality is not a property the Metal backend can promise, issue #71).
