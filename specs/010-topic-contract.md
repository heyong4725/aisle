...
| `seg_overhead` | out | Int32[h*w] | 15 Hz | per-pixel entity id; L1 ONLY (TC-9). 0 = background. Ids are the SCENE's segmentation map, not entity indices — see TC-9 |
...
- TC-9: (perception ladder — the rung is a CONTRACT, not a convention; wording is declarative pre-implementation, and the env-change PR that implements it upgrades this ID to RFC-2119 together with its citing tests, keeping the M0-4 trace gate green) `perception: L0|L1|L2` selects which pose source a graph may consume, and the bridge publishes only what the rung permits. **L0** — the graph may consume `poses` (non-privileged ground-truth pose, TC-2 layout); no segmentation is rendered. **L1** — `poses` is forbidden to every node; the bridge additionally publishes `seg_overhead`, and pose is ESTIMATED from segmentation plus `depth_overhead`. **L2** — `seg_overhead` is forbidden too and perception runs on `rgb_*` alone (deferred; T2's rung). `seg_overhead` is rendered ONLY at L1 because a segmentation pass costs an extra render on every overhead tick and BRG-2 already rate-limits depth for that reason: an L0 run's render budget is unchanged by this topic existing. Ids in `seg_overhead` are the simulator's own segmentation map (Genesis: `scene.segmentation_idx_dict`, seg id → (entity_idx, link_idx)) and are NOT entity indices — a consumer that masks on entity index silently selects other geometry, measured as robot links whose pixel counts were identical across scenes with different object layouts. The bridge therefore publishes the per-med id map in `bridge_info` so a consumer never guesses it. Rung selection is declared IN THE GRAPH, so the graph hash attests which pose source a result used, and is never inherited from ambient environment. An L1 pose estimate carries its supporting mask size so a consumer can refuse a partially occluded object rather than accept a centroid biased toward the visible fragment (measured: 2.2 mm mean XY error over 20 objects, but 19.5 mm on the smallest mask).
